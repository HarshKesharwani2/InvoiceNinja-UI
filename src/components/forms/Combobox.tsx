...
export function ComboboxAsync<T = any>({
  ...
  const [filteredEntries, setFilteredEntries] = useState<Entry<T>[]>([]);
  ...
  useEffect(() => {
    if (data) {
      setEntries([...data]);
      setFilteredEntries([...data.slice(0, 20)]); // Limit to 20 items initially
    }
  }, [data]);
  ...
  const onEmptyValues = (query: string) => {
    ...
    // Filter on the full dataset and update the displayed entries
    if (query.length > 0) {
      const filteredData = entries.filter(entry =>
        entry.label.toLowerCase().includes(query.toLowerCase())
      );
      setFilteredEntries(filteredData.slice(0, 20)); // Limit filtered results to 20
    } else {
      setFilteredEntries(entries.slice(0, 20)); // Reset to initial 20 items
    }
  };
